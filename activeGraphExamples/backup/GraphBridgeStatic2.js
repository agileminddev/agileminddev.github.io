...
*/
    ...
    if(gb.includeGutter === true){
        ...
        console.log("gb.minMaxAdj.minX: "+gb.minMaxAdj.minX);
        console.log("gb.minMaxAdj.maxX: "+gb.minMaxAdj.maxX);
        console.log("gb.minMaxAdj.minY: "+gb.minMaxAdj.minY);
        console.log("gb.minMaxAdj.maxY: "+gb.minMaxAdj.maxY);
   ...
   // this.jsxboards.push(board);
    ...
    if(gb.isCentered === true){
        ...
        if(gb.customOrigin){
            ...
            console.log("(Math.abs(gb.minX)*gb.unitX): "+(Math.abs(gb.minX)*gb.unitX));
            console.log("(Math.abs(gb.minX)*gb.unitY): "+(Math.abs(gb.maxY)*gb.unitY));
            graphBridgeStatic_obj.origin = {scrCoords:[1, (Math.abs(gb.minX)*gb.unitX), (Math.abs(gb.maxY)*gb.unitY)  ]};
        }
    ...
    graphBridgeStatic_obj.makePoint = function(obj){
     ...
     graphBridgeStatic_obj.makePointsAndLine = function(obj){
        ...
        for(i=0; i< obj.pts.length; i++){
         ...
         graphBridgeStatic_obj.makeLine({ptArray:[ptStart,ptEnd], straightFirst:false, straightLast:false, fixed:gb.confObj.fixedObjects});
    };
